...
/**
 * Строится уравнение прямой проходящей через каждые две точки маршрута. Линиями, перпендикулярными данной прямой,
 * проведенными через начало и кончало отрезка, а так же линиями, параллельными данной прямой, удаленными на заданное
 * расстояние ограничивается область поиска.
 * Для точек, попавших в эту область находится проекция на отрезок маршрута. Сортируются точки по степени удаления их
 * проекций от начальной точки отрезка маршрута.
 */
